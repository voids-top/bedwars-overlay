...
class window_handler:
    def __init__(self):
        ...
        self.move = False
        self.moving = None
        self.opacity = int(config["opacity"])/100
        ...
        self.opacity_position = int(config["opacity"]-10)*3*10/9
    ...
    def move_app(self,event):
        if self.moving == "1":
            if event.x < 100:
                config["opacity"] = 10
                self.opacity_position = 0
            if 100 <= event.x <= 400:
                value = event.x-100
                config["opacity"] = float(value/3/10*9)+10
                self.opacity = int(config["opacity"]) / 100
                self.opacity_position = value
            if event.x > 400:
                config["opacity"] = 100
                self.opacity_position = 300
        if self.moving == "2":
            if event.x < 100:
                config["fps"] = 1
                self.fps_position = 0
        ...
        while True:
            if self.now != self.current:
                ...
                if self.now == "about":
                    ...
                    window.settings(self.canvas, config, self.opacity_position, self.fps_position, self.refresh_position)
                else:
                    self.now = None
                    self.current = None
    ...
    def loop(self):
        self.frames += 1
        self.tk.attributes('-alpha', self.opacity)
